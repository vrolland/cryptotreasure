...
function toBytes32(number) {
  return ethers.utils.hexZeroPad(
    ethers.utils.hexValue(BigNumber.from(number)),
    32
  );
}
...
function addTypeParamToBytes(
  addressErc20ToLock = 0,
  amountToLock = 0,
  durationLockDestroy = 0,
  mintingDurationLock = 0,
  numberReserved = 0
) {
  ...
  return ethers.utils.hexConcat([
    ...
    amountToLockHex,
    durationLockDestroyHex,
    mintingDurationLockHex,
    numberReservedHex,
  ]);
}
...
function parseDataTypes(data) {
  ...
  const amountToLock = BigNumber.from(
    `0x${data.slice(43, 43 + 64)}`
  ).toString();
  const durationLockDestroy = BigNumber.from(
    `0x${data.slice(106, 106 + 64)}`
  ).toString();
  const mintingStartingTime = BigNumber.from(
    `0x${data.slice(170, 170 + 64)}`
  ).toString();
  const numberReserved = BigNumber.from(
    `0x${data.slice(234, 234 + 64)}`
  ).toString();
  return {
    tokenToLock,
    amountToLock,
